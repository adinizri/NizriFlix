...
for dirpath,dirnames,filenames in os.walk('G:/Movies&Series/Movies/all'):
    print(filenames )
    names=filenames
    ...
    for name in names:
         ...
         if 'TorrentPart' in newName:
            ...
            if 'mp4' in newName:
                ...
                if '1080p'in newName:
                  newName=newName.replace((newName[newName.index('1080p'):len(newName)-1]),"") 
                if '720p'in newName:
                  newName=newName.replace((newName[newName.index('720p'):len(newName)-1]),"") 
                if(' 4 ' in newName) :
